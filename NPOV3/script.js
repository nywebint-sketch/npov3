...
// 2) Назови файл logo.png (или поменяй все src="logo.png" на свое имя)
...
function renderReleases() {
  ...
  [...data.releases].sort((a, b) => b.date.localeCompare(a.date)).forEach((release) => {
    ...
    card.appendChild(createMedia("logo.png", "", "media square"));
...
function buildReleaseModalBody(release) {
  ...
  const left = el("div", { className: "card event-modal-left" });
  left.appendChild(createMedia("logo.png", "", "media"));
...
function buildStreamModalBody(stream) {
  ...
  appendDivider(card);
  card.appendChild(createMedia("logo.png", "", "media wide"));
...
function buildMerchModalBody(item) {
  ...
  left.appendChild(createMedia("logo.png", "", "media square"));
...
document.addEventListener("click", (e) => {
  ...
  if (type === "podcast") {
    ...
    appendDivider(left);
    left.appendChild(createMedia("logo.png", "", "media wide"));
